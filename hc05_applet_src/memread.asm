***************************************************************
* MEMREAD.ASM
* Applet for PROG05 to read any address within the HC05 memory map
* Author: Sonic2k
* Date: 7 May 2023
*
...
********************************************
    org $BA
opcode    ds      1     ; LDA nnnn,X
addrhi    ds      1
addrlo    ds      1     ; high and low address in memory map
return    ds      1     ; RTS
...
Loop:
     ...
     ; Read location and return the value at that location
        LDA     #$00
        TAX
        JSR     $BA
;        LDA     $00FF,X    ; operand is "base address" and X is the offset it would seem
        JSR     Transmit        ; So far this works, we will need to create an overlay in RAM to create a function
                                ; that can overlay LDA opr,X to read any address from 0x0000 ~ 0xFFFF
        BRA     Loop
